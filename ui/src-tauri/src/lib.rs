...
fn translate_unit_in_project(
    ...
    let (locale, glossary) = services::translate::resolve_project_translate_context(&state, &abs)?;
    let backend =
        OllamaBackend::new().map_err(|e| format!("ollama backend construction failed: {e}"))?;
    let backend_name = backend.name().to_string();

    services::translate::translate_one(
...
async fn translate_glossary_term(
    ...
    let (term_source, glossary, locale) = {
        ...
        let term_source = services::translate::glossary_term_source(&glossary, &term_id)?;
        (term_source, glossary, locale)
    };

    let backend =
        OllamaBackend::new().map_err(|e| format!("ollama backend construction failed: {e}"))?;
    services::translate::dispatch_glossary_term_translation(
        &backend,
        &term_source,
        locale,
        &glossary,
...
fn translate_batch_in_project(
    ...
    let unit_ids: Vec<UnitId> = {
        ...
        entry
            ...
            .units()
            .iter()
            .filter(|u| services::translate::unit_matches_scope(u, scope))
            .map(|u| u.id.clone())
            .collect()
    };
...
fn run_batch_worker(
    ...
    for unit_id in &unit_ids {
        ...
        match services::translate::translate_one(
            &state.project_catalogs,
            &state.project,
            &abs,
            unit_id,
